    ...
    <G as ToSliceRepresentation>::SliceRepresentation: ReduceGraphNodes + SelfLoop + Default,
{
    ...
    merge_trees: IntersectionForest<<G as ToSliceRepresentation>::SliceRepresentation>,
    ...
    pub fn new(graph: &'a mut G, mut initial_solution: DominatingSet, rng: &'a mut R) -> Self {
        ...
        if initial_solution.all_fixed() {
            return Self {
                ...
                scores: Vec::new(),
                age: Vec::new(),
                merge_trees: IntersectionForest::new(
        ...
        let mut merge_trees = IntersectionForest::new_sorted(neighborhoods);
